...
export class StripeWebhookController
{
  ...
  )
  {
    ...
    const event = this.stripePayloadService.tryHydratePayload(sig, rawBody, "account");
  ...
  )
  {
    ...
    const event = this.stripePayloadService.tryHydratePayload(sig, rawBody, "connect");

    await this.stripeWebhookService.handleConnectWebhook(event);
  }
}
